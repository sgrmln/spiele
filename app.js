...
async function loadGames() {
  ...
  if (error) { sync('error'); toast('Ladefehler: ' + error.message, true); return; }
  games = data || [];
  sync('ok');
  render();
}
...
function cardHTML(g) {
  ...
  return `<div class="game-card${g.favorit ? ' favorit' : ''}" onclick="openPanel(${g.id})">
    <div class="card-cover">
      <span class="card-icon">${icon(g.id)}</span>
    </div>
    <div class="card-btns">
...
function rowHTML(g) {
  ...
  const sub   = [g.verlag, g.autor ? g.autor.split(',')[0].trim() : '', g.gruppe, g.jahr]
                  .filter(Boolean).join(' · ');
...
function render() {
  ...
  keys.forEach(k => {
    html += `<div class="group-block">
      <div class="group-title">${k} <span class="group-count">${groups[k].length} Titel</span></div>
      <div class="game-list">${groups[k].map(rowHTML).join('')}</div>
    </div>`;
  });
  if (einzel.length) {
    html += `<div class="group-block">
      <div class="group-title">Einzeltitel <span class="group-count">${einzel.length}</span></div>
